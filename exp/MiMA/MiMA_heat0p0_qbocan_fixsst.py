...
exp = Experiment('mima_heat0p0_qbocan_fixsst', codebase=cb)

exp.inputfiles = [os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'), os.path.join(GFDL_BASE,'input/sst_ctrl_clim.nc'), os.path.join(GFDL_BASE,'input/qbo_canonical_cycle.txt')]
...
exp.namelist = namelist = Namelist({
    ...
    'damping_driver_nml': {
        ...
        'do_ewa_htg': False,
        'do_sin_qbo': True,   # enables the nudging call; the target below is the table
        # canonical observed QBO cycle instead of the symmetric sinusoid
        'do_tab_qbo': True,
        'qbo_table_file': 'INPUT/qbo_canonical_cycle.txt',
        'qbo_tab_scale': 1.0,
    },
