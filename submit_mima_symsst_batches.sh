#!/bin/bash
#
# Submit the symmetric-SST subset + underworld-heating experiments (600 months) in batches of 4 to SLURM
...
declare -a all_experiments=(
    "MiMA_heat0p0_noqbo_symsst"
    "MiMA_heat0p1_latcent00B_pcent030_noqbo_symsst"
    "MiMA_heat0p1_latcent00B_pcent050_noqbo_symsst"
    "MiMA_heat0p1_latcent30B_pcent050_noqbo_symsst"
    "MiMA_heat0p1_latcent45S_pcent050_noqbo_symsst"
    "MiMA_heat0p1_latcent45S_pcent125_noqbo_symsst"
    "MiMA_heat0p1_latcent60B_pcent050_noqbo_symsst"
    "MiMA_heat0p1_latcent60B_pcent070_noqbo_symsst"
    "MiMA_heat0p1_latcent60S_pcent125_noqbo_symsst"
    "MiMA_heat0p1_latcent75S_pcent125_noqbo_symsst"
    "MiMA_heat0p1_latcent90B_pcent050_noqbo_symsst"
    "MiMA_heat0p1_latcent90S_pcent050_noqbo_symsst"
)
...
for exp_name in "${all_experiments[@]}"; do
    ...
    run0600_dir="$GFDL_DATA/$exp_dir_name/run0600"

    if [ ! -d "$run0600_dir" ]; then
...
for ((batch=0; batch<num_batches; batch++)); do
    batch_num=$((batch + 1))
    batch_script="run_mima_symsst_batch${batch_num}.sh"

    # Start batch script
    cat > "$batch_script" << 'EOF_HEADER'
#!/bin/bash
#SBATCH --job-name=mima_symsst
#SBATCH --nodes=1
#SBATCH --ntasks=4
...
EOF_HEADER
    ...
    echo "#SBATCH -o mima_symsst_batch${batch_num}_%j.txt" >> "$batch_script"
    echo "#SBATCH -e mima_symsst_batch${batch_num}_%j.err" >> "$batch_script"
    echo "" >> "$batch_script"
...
for ((batch=0; batch<num_batches; batch++)); do
    batch_num=$((batch + 1))
    echo "  run_mima_symsst_batch${batch_num}.sh"
done
echo ""
